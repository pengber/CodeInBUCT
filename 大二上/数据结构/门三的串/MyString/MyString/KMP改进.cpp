 ...
 *KMP改进注意事项:
 *还是以上面那个子串为例:
 *假设我字符在P[5](C)失效了,那应该比较主串的下一个字符与p[next[5](2)](C)的相同与否,但C不同,继续比较与p[0]
 *的相同与否,注意到没有,因为你p[5] == p[2],所以p[5]失效了你比较p[2]肯定失效,所以你就多比较了一步,也就是说,
 *如果子串中的后面的某个字符的next值与前面相同的字符的next值是有关的,而且是相同的,这样你在求next值的时候,
 *再加一个比较语句,直接求出p[5]的最好的next值,一步到位这样最好。
*/

void computeNextval(char * p, int * nextval) {
	int i = 0;		
	int j = -1;
	nextval[0] = -1;
...
void KMP(char * s, char * p, int * nextval) {
	int i = 0;
	int j = 0;
	...
	while (i < sLength && j < pLength) {
		if (j < 0 || s[i] == p[j]) {
			...
			j = nextval[j];					//这句是精髓，也就是当不匹配时，j置为下次
