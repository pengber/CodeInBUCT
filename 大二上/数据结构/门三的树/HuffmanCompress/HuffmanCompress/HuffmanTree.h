...
void HuffmanTree<CharType, WeightType>::createHuffmanTreeHelp(CharType * chars, WeightType * weight, int size) {
	...
	for (pos = 1; pos <= m_size; pos++) {
		...
		average += weight[pos - 1] * charCode.getLength();
		comentropy += weight[pos - 1] * log(weight[pos - 1]) / log(2);
		std::cout << chars[pos-1] << "字符的权重为：" << weight[pos-1] << "编码是：";
	...
	comentropy = 0 - comentropy;
	int tempCom = (int)(comentropy * 100);
	comentropy = (float)tempCom / 100;
	std::cout << "信息熵为:" << comentropy << std::endl;
	std::cout << "平均码长为:" << average << std::endl;
	std::cout << "信息冗余量为:" << (average - comentropy) / average << std::endl;
	m_curPos = nodeCount;
