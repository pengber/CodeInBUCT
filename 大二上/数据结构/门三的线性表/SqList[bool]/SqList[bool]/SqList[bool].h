...
//模板类特征成员函数声明
	...
	bool isExitByVal(int & pos, const ElemType & elem) const;//按值查找得位置并通过引用参数返回
...
//初始条件:无重名类对象
//操作结果:构造一个容量为capacity的线性表，实际上容量为99，因为insert和set都是以1为起始的，也就是说第一个的物理地址为elems[1]，而不是elems[0]
template <class ElemType>
