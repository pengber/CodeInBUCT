#ifndef ADJMATRIXUNDIRGRAPH_H_INCLUDE
#define ADJMATRIXUNDIRGRAPH_H_INCLUDE
#include <iostream>
...
void AdjMatrixUndirGraph<ElemType>::DFSHelp(int vertex, void(*visit)(ElemType &)) {
	...
	if (m_visited[vertex] == UNVISITED) {			//其实这个判断语句没有必要,因为进入此函数必然没有访问过
...
void AdjMatrixUndirGraph<ElemType>::DFSTraverse(int vertex, void(*visit)(ElemType &)) {
	...
	for (int i = vertex; i < m_vertexSize; i++) {
		DFSHelp(i, visit);				//如果这里有判断没有访问过的话再调用DFSHelp则DFShelp中就不需要了
	}
}
template <class ElemType>
