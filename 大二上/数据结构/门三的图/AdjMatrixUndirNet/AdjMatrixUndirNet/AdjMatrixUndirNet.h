...
//函数功能: 实现广度优先搜索
template <class ElemType, class WeightType>
void AdjMatrixUndirNet<ElemType, WeightType>::BFSHelp(int vertex, void(*visit)(ElemType &)) {
#ifdef DEBUG
	std::cout << "FUNC:" << __func__ << ",LINE:" << __LINE__ << std::endl;
#endif // DEBUG
	if (m_visited[vertex] == UNVISITED) {
...
inline void AdjMatrixUndirNet<ElemType, WeightType>::Prim(ElemType vertex)
{
	...
	this->setVisited(k);					//严老师用lowCost = 0表示V集合,我用visited[k] = true并且设置其closege[k] = INFINITY为V集合
	for (int i = 0; i < this->getVertexCount(); i++) {
		closedge[i].vertex = vertex;
		closedge[i].weight = m_matrix[k][i];
	}
											//严老师用的是closedge[k].lowcost = 0 划分集合
	for (int j = 1; j < m_vertexSize; j++) {
		WeightType tempWeight = INFINITY;
		for (int z = 0; z < m_vertexSize; z++) {
			if (tempWeight > closedge[z].weight) {
				k = z;
				tempWeight = closedge[z].weight;
			}
		}
		allWeight += closedge[k].weight;	//OJ平台的题
		//cout << "连接:" << m_elems[k] << "其权重:" << closedge[k].weight << endl;
		this->setVisited(k);				//这句话没必要???为啥?
		closedge[k].weight = INFINITY;		//这句话不能删去,因为不修改的话,weight里面最小的总是第一个k
	
		for (int z = 0; z < m_vertexSize; z++) {
			if (closedge[z].weight > m_matrix[k][z] && !this->getVisited(z)) {//&&后面这个判断条件挺重要的
				closedge[z].weight = m_matrix[k][z];
				closedge[z].vertex = m_elems[k];
			}
		}
	}
	cout << allWeight << endl;				//这行是为了OJ平台的题
}
template<class ElemType, class WeightType>
inline void AdjMatrixUndirNet<ElemType, WeightType>::kruskal()
{
	kruskalForest forest(m_vertexSize);
	kruskalEdge<WeightType> * edges = new kruskalEdge<WeightType>[m_edgeSize];
	int countt = 0;
	int weightSum = 0;
	for (int i = 0; i < m_vertexSize; i++) {
		for (int j = this->getFirstAdjVertex(i); j != -1; j = this->getNextAdjVertex(i, j)) {
			if (i > j) {
				kruskalEdge<WeightType> tempEdge(i, j, m_matrix[i][j]);
				edges[countt++] = tempEdge;
			}
		}
	}
	
	for (int i = 0; i < countt - 1; i++) {
		for (int j = 0; j < countt - 1 - i; j++) {
			kruskalEdge<WeightType> tempEdge;
			if (edges[j].m_weight > edges[j + 1].m_weight) {
				tempEdge = edges[j];
				edges[j] = edges[j + 1];
				edges[j + 1] = tempEdge;
			}
	...
	for (int i = 0; i < countt; i++) {
		int vex1 = edges[i].m_vex1;
		int vex2 = edges[i].m_vex2;

		if (!forest.isSameTree(vex1, vex2)) {
			//std::cout << vex1 << "<->" << vex2 << <<":" << edges[i].m_weight << std::endl;
			weightSum = weightSum + edges[i].m_weight;
			forest.unionTree(vex1, vex2);
		}
	}
