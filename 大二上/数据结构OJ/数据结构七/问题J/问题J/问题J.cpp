...
public:
	...
	void shortestPathDij(int vertex, WeightType *path, AdjMatrixDirNet<int, int> & netTwo, WeightType * pathTwo);
...
template <class ElemType, class WeightType>
void AdjMatrixDirNet<ElemType, WeightType>::shortestPathDij(int vertex, WeightType * path, AdjMatrixDirNet<int, int> & netTwo, WeightType * pathTwo) {
	bool p[1000][1000];                             //privotal关键的,表征是否为vertex到达某一点的必经点
	for (int i = 0; i < m_vertexSize; i++) {
		m_visited[i] = UNVISITED;
		path[i] = m_matrix[vertex][i];
		pathTwo[i] = netTwo.m_matrix[vertex][i];
	...
	for (int i = 1; i < m_vertexSize; i++) {
		...
		WeightType minTwo = INFINITY;
		for (int j = 0; j < m_vertexSize; j++) {
			if (!m_visited[j] && path[j] < min && pathTwo[j] < minTwo) {
				min = path[j];
				minTwo = pathTwo[j];
				v = j;
			}
		...
		for (int w = 0; w < m_vertexSize; w++) {
			if (!m_visited[w] && (min + m_matrix[v][w] < path[w]) && (minTwo + netTwo.m_matrix[v][w] < pathTwo[w])) {
				path[w] = min + m_matrix[v][w];
				pathTwo[w] = minTwo + netTwo.m_matrix[v][w];
				for (int i = 0; i < m_vertexSize; i++) {
					p[w][i] = p[v][i];
				}
				p[w][w] = true;
...
int main() {
	...
	while (cin >> n >> m && n && m) {
		...
		int start, end;
		int * elems = new int[n];
		int * shortLen = new int[n];
		int * lowCost = new int[n];

		AdjMatrixDirNet<int, int> netOne;
		AdjMatrixDirNet<int, int> netTwo;

		for (int i = 0; i < n; i++) {
			elems[i] = i;
		}
		netOne.setGraph(elems, n);
		netTwo.setGraph(elems, n);

		for (int i = 0; i < m; i++) {
			cin >> a >> b >> c >> d;
			netOne.insertEdge(a - 1, b - 1, c);
			netOne.insertEdge(b - 1, a - 1, c);
			netTwo.insertEdge(a - 1, b - 1, d);
			netTwo.insertEdge(b - 1, a - 1, d);
		}
		cin >> start >> end;
		netOne.shortestPathDij(start - 1, shortLen, netTwo, lowCost);
		
		cout << shortLen[end - 1] << " " << lowCost[end - 1] << endl;
	}
