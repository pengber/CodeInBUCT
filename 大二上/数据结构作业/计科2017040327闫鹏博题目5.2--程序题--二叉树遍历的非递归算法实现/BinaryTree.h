...
//特征函数
	...
	void postCreate(void(*visit)(ElemType &), bool(*isNull)(ElemType &));
...
template <class ElemType>
int BinaryTree<ElemType>::getNodeCountHelp(const BinTreeNode<ElemType> * &root) const {
#ifdef DEBUG
...
template <class ElemType>
int BinaryTree<ElemType>::getHeightHelp(const BinTreeNode<ElemType> * &root) const {
#ifdef REMIND
	std::cout << "FUN:" << __func__ << ",LINE:" << __LINE__ << std::endl;
#endif // REMIND
	if (root == NULL) {
...
void BinaryTree<ElemType>::inCreateHelp(BinTreeNode<ElemType> * &root, void(*visit)(ElemType &), bool(*isNull)(ElemType &)) {
	...
	if ((*isNull)(elem)) {
		...
		root = BinTreeNode<ElemType>();

		inCreateHelp(root->left, visit, isNull);
		root->date = elem;
		inCreateHelp(root->right, visit, isNull);
	}
}
template <class ElemType>
void BinaryTree<ElemType>::postCreateHelp(BinTreeNode<ElemType> * &root, void(*visit)(ElemType &), bool(*isNull)(ElemType &)) {
	...
	if ((*isNull)(elem)) {
		...
		postCreateHelp(root->left, visit, isNull);
		postCreateHelp(root->right, visit, isNull);
		root->date = elem;
	}
}
template <class ElemType>
...
template <class ElemType>
ElemType BinaryTree<ElemType>::getElem(const BinTreeNode<ElemType> * &cur) const {
	if (cur == NULL) {
...
void BinaryTree<ElemType>::insertLeftChild(BinTreeNode<ElemType> * &cur, const ElemType & elem) {
	if (cur == NULL) {
		...
		newNode = new BinTreeNode<ElemType>(elem);
...
void BinaryTree<ElemType>::levelOrder(void(*visit)(ElemType &)) {
	...
	while (!queue.isEmpty()) {
		...
		(*visit)(tempNode->data);
		if (tempNode.leftChild != NULL) queue.inQueue(queue->leftChild);
		if (tempNode.rightChild != NULL) queue.inQueue(queue->rightChild);
	}
}
template <class ElemType>
