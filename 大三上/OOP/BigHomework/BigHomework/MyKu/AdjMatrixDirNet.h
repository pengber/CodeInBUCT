...
int ve[1000] = { 0 };						//关键路径需要,其含义为事件发生最早时间
int vl[1000] = { 0 };						//关键路径需要,其含义为事件发生最晚事件
//#define REMIND
//#define DEBUG
...
public:
	...
	bool topologicalForCriticalPath();

	bool criticalPath();

	void shortestPathFord(WeightType pathNet[][100], int pathInfo[][100]);
...
//函数名称: bool topologicalSort()
//函数功能: 输出拓扑排序序列,成功返回true;有环返回fasle;
template <class ElemType, class WeightType>
bool AdjMatrixDirNet<ElemType, WeightType>::topologicalSort() {
	...
	while (!stack.isEmpty()) {
		...
		for (p = this->getFirstAdjVertex(tempVertex); p != -1; p = this->getNextAdjVertex(tempVertex, p)) {
			if ((--degree[p]) == 0) {
			stack.push(p);				//如果有环的话有节点degree不会减少为0而入栈被访问,所以count会小于m_vertexSize
			}
		}
	}
...
//函数名称: bool topologicalForCriticalPath()
//函数功能: 得到事件发生最早时间的数组,提供给criticalPath()函数,有环失败返回false
template<class ElemType, class WeightType>
inline bool AdjMatrixDirNet<ElemType, WeightType>::topologicalForCriticalPath()
{
	LinkStack<int> stack;
	int count = 0;							//及拓扑排序中输出的顶点数
	int degree[1000] = { 0 };
	...
	while (!stack.isEmpty()) {
		...
		if (ve[tempVertex] + m_matrix[tempVertex][p] > ve[p]) ve[p] = ve[tempVertex] + m_matrix[tempVertex][p];
		}
	}
...
//函数名称: bool criticalPath()
//函数功能: 输出关键路径
//注意事项: 此题思路是拓扑排序后再★逆向★用拓扑排序求得事件发生最晚时间,不用栈
//			我真是个小机灵鬼儿
template<class ElemType, class WeightType>
inline bool AdjMatrixDirNet<ElemType, WeightType>::criticalPath()
{
	if (!topologicalForCriticalPath()) return false;
	
	LinkStack<int> stack;
	int r_matrix[100][100];			//逆向数组,反向拓扑排序
	int degree[1000] = { 0 };
	int tempVertex;
	int p;
	int ee, el;						//事件最早最晚开始时间
	char tag;
	for (int i = 0; i < m_vertexSize; i++) {
		vl[i] = ve[i];
	}
	...
	while (!stack.isEmpty()) {
		...
		for (p = this->getFirstAdjVertex(tempVertex); p != -1; p = this->getNextAdjVertex(tempVertex, p)) {
			...
			if (vl[tempVertex] - m_matrix[tempVertex][p] < vl[p]) vl[p] = vl[tempVertex] - m_matrix[tempVertex][p];
		}
	}
	std::cout << "*表示关键路径,!表示不是!" << std::endl;
	for (int i = 0; i < m_vertexSize; i++) {
		for (p = this->getFirstAdjVertex(i); p != -1; p = this->getNextAdjVertex(0, p)) {
			ee = ve[i]; el = vl[p] - m_matrix[i][p];
			tag = (ee == el) ? '*' : '!';
			std::cout << i << "->" << p << " " << tag << std::endl;
		}
	}
...
template<class ElemType, class WeightType>
void AdjMatrixDirNet<ElemType, WeightType>::shortestPathFord(WeightType pathNet[][100], int pathInfo[][100])
{
	for (int i = 0; i < m_vertexSize; i++) {
		for (int j = 0; j < m_vertexSize; j++) {
